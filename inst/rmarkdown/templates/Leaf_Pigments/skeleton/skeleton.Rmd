...
`r #note that any data chunks below that you don't want in your lab report can simply be deleted.  If you need to add results chunks, feel free to add as needed.`
...
# Table 1 - A table showing the mobile phase results.  Note: If a pigment is missing a data point for any reason (not taken or pigment not present), then use "NA" in place of the value.
Mobile_phase <- c("100 % Cyclohexane") #Input the ratio of mobile phase components that were tested
Rf_Anthocyanin <- c() #Input the calculated Rf values of anthocyanin for the different mobile phase tests
Rf_ChlorA <- c() #Input the calculated Rf values of chlorophyll A for the different mobile phase tests
Rf_ChlorB <- c() #Input the calculated Rf values of chlorophyll B for the different mobile phase tests
Rf_Xan <- c() #Input the calculated Rf values of xanthophyll for the different mobile phase tests
Rf_Car <- c() #Input the calculated Rf values of carotenoid for the different mobile phase tests
...
```{r, results = 'asis', echo=FALSE}
# Table 2 - A table showing the chromatogram results.  Note: If a pigment is missing a data point for any reason (not taken or pigment not present), then use "NA" in place of the value.
...
```{r, echo=FALSE, fig.cap="Insert Caption"}
#Here is where you input the code for a spectroscopy picture.  You don't need to include all your pictures, since the data is reported in the table, but you may want to include at least one as a example of what you were looking at.  
...
Pigment <- c("Anthocyanin", "Chlorophyll A", "Chlorophyll B", "Xanthophyll", "Carotenoid")
Spec_Control<- c() #This is the lambda max from the UV/VIS spectrum of the control pigments. If there are multiple lambda max values, you can use quotes such as c("252, 298", "520").  This shows the first pigment has two lambda max values at 252 and 298 nm while the second pigment only has a lammbda max at 520 nm.
